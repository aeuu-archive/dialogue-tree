...
message()
{
  TITLE="Cannot start DialogueTree"
...
# Try (in order): DialogueTree_JDK, dialoguetree.jdk, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$DialogueTree_JDK" -a -x "$DialogueTree_JDK/bin/java" ]; then
  JDK="$DialogueTree_JDK"
elif [ -s "$HOME/.DialogueTree1.0/config/dialoguetree.jdk" ]; then
  JDK=`"$CAT" $HOME/.DialogueTree1.0/config/dialoguetree.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JDK found. Please validate either DialogueTree_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  exit 1
fi
...
# ---------------------------------------------------------------------
if [ -n "$DialogueTree_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$DialogueTree_PROPERTIES"
fi
...
elif [ -r "$HOME/.DialogueTree1.0/config/mps$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.DialogueTree1.0/config/mps$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/dialoguetree$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/dialoguetree$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/dialoguetree$BITS.vmoptions"
fi
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=DialogueTree1.0"
  fi
fi
...
if [ -n "$DialogueTree_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$DialogueTree_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=DialogueTree1.0
